...
//
    ...
    let pins = (
        gpioa.pa0.into_alternate_push_pull(&mut gpioa.ctl0),
        gpioa.pa1.into_alternate_push_pull(&mut gpioa.ctl0),
        gpioa.pa2.into_alternate_push_pull(&mut gpioa.ctl0),
        gpioa.pa3.into_alternate_push_pull(&mut gpioa.ctl0),
    );

    let mut tmr4 = Timer::timer4(dp.TIMER4, clocks, &mut rcu.apb1);
    let mut pwm = tmr4.pwm(pins, 10.khz());
        // .pwm(pins, &mut afio.mapr, 10.khz());
